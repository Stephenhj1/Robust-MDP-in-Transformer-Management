        ...
        for i in X:
            ...
            if (t % 2 == 0 and main_or_intermediate(i, F_a1) == "Main State") or (t % 2 != 0 and main_or_intermediate(i, F_a1) == "Intermediate State"):
                ...
                for a in A:
                    ...
                    result = np.sum(f[valid_indices] / (mu - V_prev[valid_indices]))
                    if result == 0:
                        lambda_mu = 0
                    else:
                        lambda_mu = 1 / result
                    ...
                    if lambda_mu == 0:
                        ...
                        sigma = mu - ( 1+beta ) * lambda_mu + lambda_mu * np.sum(f[valid_indices] * np.log ( (lambda_mu * f[valid_indices]) / (mu - V_prev[valid_indices]) ) )
                ...
                # Set the value function to the previous value function
                V[i] = V_prev[i]
                policy[t][i] = None
...
if __name__ == "__main__":
    ...
    df_policy.to_csv('policy_table.csv')
